...
- Agent internals: where the state machine object lives inside the Pipecat pipeline, and the exact structured-output schema the LLM uses to propose transitions.
- Text mode: does it reuse the same `session.create` path with a text transport, or bypass WebRTC entirely?
